...
## 2. API Xem danh sách Vật tư (Supplies List)
- **Endpoint:** `GET /api/manager/supplies`
- **Mô tả:** Trả về danh sách mẫu vật tư cứu trợ kèm tổng tồn kho thực tế.

## 3. API Xem lịch sử Nhập/Xuất (Stock History)
- **Endpoint:** `GET /api/manager/supplies/history`
- **Mô tả:** Trả về nhật ký toàn bộ các giao dịch nhập và xuất kho. Đặc biệt quan tâm đến:
    - **Cột Body:** Danh sách các mặt hàng (ID) và số lượng xuất/nhập.
    - **Cột Note:** Ghi chú chi tiết về mục đích sử dụng vật tư.

## 4. Dữ liệu mẫu (Gợi ý test)
